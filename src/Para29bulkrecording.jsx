...
function Para29BulkRecording() {
    ...
    // Fetch next group of unrecorded ayats (Surah-wise) for Para 29
    const fetchNextAyatsGroup = async () => {
        try {
            setLoading(true);
            const token = localStorage.getItem('token');
            const response = await fetch(`https://qurandatasetapp-backend-1.onrender.com/api/para29-bulk-recording/next`, {
            ...
            if (response.ok) {
                if (data.ayats && data.ayats.length > 0) {
                    ...
                    setAyatsGroup([]);
                    setUserProgress({
                        recorded: data.userRecorded || 0,
                        total: data.totalAyats || 0
                    });
                }
            } else {
    ...
    const saveAllRecordings = async () => {
        ...
        try {
            ...
            for (const [ayatIndex, audioBlob] of recordingsList) {
                ...
                try {
                    const response = await fetch('https://qurandatasetapp-backend-1.onrender.com/api/para29-memorization/save', {
                        method: 'POST',
                        headers: { Authorization: `Bearer ${token}` },
                        body: formData
                    });
        ...
        return (
            <div className="container">
                ...
                <div className="complete-message">
                    ...
                    <p>You have successfully recorded all {userProgress.total} ayats from Para 29.</p>
                </div>
            </div>
        );
    }
                ...
                <div className="progress">
                    ...
                    <div className="progress-bar">
                        <div
                            ...
                            style={{ width: `${userProgress.total > 0 ? (userProgress.recorded / userProgress.total) * 100 : 0}%` }}
                        ></div>
                    </div>
...
export default Para29BulkRecording;